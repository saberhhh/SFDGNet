...
def visualization(visu, visu_format, test_choice, data, seg, pred, visual_file_index, semseg_colors):
    ...
    for i in range(0, data.shape[0]):
        ...
        if skip:
            ...
            data = np.loadtxt(
                'data/indoor3d_sem_seg_hdf5_data_test/raw_data3d/Area_' + test_area + '/' + roomname + '(' + str(
...
def train(args, io):
    writer = SummaryWriter("train_log/log_TestL1")
    ...
    regularization = sparse_regularization(model=model,device=device)
    ...
    for epoch in range(args.epochs):
        ...
        for data, seg in loop:
            ...
            loss += lambad2*regularization.hierarchical_squared_group_l12_regularization(lambda1)
            loss += (1-lambad2)*regularization.l1_regularization(lambda1)
            ...
            loop.set_postfix(loss=(train_loss * 1.0))
        ...
        train_sparsity = sparsity(model)
        outstr = 'Train %d, loss: %.6f, train acc: %.6f, train avg acc: %.6f, train iou: %.6f, sparity: %.6f' % (epoch,
        ...
        writer.add_scalar('train/sparsity',train_sparsity , epoch)
        ...
        with torch.no_grad():
            for data, seg in loop2:
                ...
                loop2.set_postfix(loss=(test_loss * 1.0))
        ...
        test_sparsity = sparsity(model)
        outstr = 'Test %d, loss: %.6f, test acc: %.6f, test avg acc: %.6f, test iou: %.6f, sparity: %.6f' % (epoch,
                                                                                              test_loss * 1.0 / count,
                                                                                              test_acc,
                                                                                              avg_per_class_acc,
                                                                                              np.mean(test_ious),
        ...
        writer.add_scalar('test/iou', np.mean(test_ious), epoch)
        writer.add_scalar('test/sparsity', test_sparsity, epoch)
...
def test(args, io):
    ...
    for test_area in range(1, 7):
        ...
        if (args.test_area == 'all') or (test_area == args.test_area):
            ...
            writer = SummaryWriter("train_log/testvis")
            ...
            test_ious = calculate_sem_IoU(test_pred_seg, test_true_seg)
            test_sparsity = sparsity(model)
            outstr = 'Test :: test area: %s, test acc: %.6f, test avg acc: %.6f, test iou: %.6f, sparity: %.6f' % (test_area,
                                                                                                    test_acc,
                                                                                                    avg_per_class_acc,
                                                                                                    np.mean(test_ious),
                                                                                                    test_sparsity)
            ...
            io.cprint("Per-class IoU for test area %s:" % test_area)
            for cls_idx, cls_name in enumerate(class_names):
                io.cprint("  %s: %.4f" % (cls_name.ljust(10), test_ious[cls_idx]))
            ...
            all_sparity.append(test_sparsity)
    ...
    if args.test_area == 'all':
        ...
        outstr = 'Overall Test :: test acc: %.6f, test avg acc: %.6f, test iou: %.6f, sparity_avg: %.6f' % (all_acc,
                                                                                         avg_per_class_acc,
                                                                                         np.mean(all_ious),
                                                                                         np.mean(all_sparity))
        io.cprint(outstr)
        io.cprint("Overall Per-class IoU:")
        for cls_idx, cls_name in enumerate(class_names):
            io.cprint("  {:10s}: {:.4f}".format(cls_name, all_ious[cls_idx]))
